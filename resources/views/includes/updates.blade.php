@foreach ($updates as $response)
		...
		@endphp
	<div class="card mb-3 card-updates rounded-large shadow-large card-border-0 @if ($response->status == 'pending') post-pending @endif @if ($response->fixed_post == '1' && request()->path() == $response->user()->username || auth()->check() && $response->fixed_post == '1' && $response->user()->id == auth()->user()->id) pinned-post @endif" data="{{$response->id}}">
	<div class="card-body">
...
<div class="card-footer bg-white border-top-0 rounded-large">
    <h4>
			...
			<a href="javascript:void(0);" @guest data-toggle="modal" data-target="#loginFormModal" @endguest class="pulse-btn btnLike @if ($likeActive)active @endif {{$buttonLike}} text-muted mr-2" @auth data-id="{{$response->id}}" @endauth>
			...
			<a href="javascript:void(0);" title="{{trans('general.share')}}" data-toggle="modal" data-target="#sharePost{{$response->id}}" class="pulse-btn text-muted text-decoration-none mr-2">
